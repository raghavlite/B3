...
class GradCache:
    ...
    def __init__(
        ...
        self.multipliers_qry = [2**i for i in range(int(np.log2(self.chunk_sizes[0])) + 1)]
        self.thresholds_qry = [self.dynamic_limit * m for m in self.multipliers_qry]

        self.multipliers_pos = [2**i for i in range(int(np.log2(self.chunk_sizes[1])) + 1)]
        self.thresholds_pos = [self.dynamic_limit * m for m in self.multipliers_pos]
    ...
    def cache_step(
        ...
        **loss_kwargs
        ...
        #! Constants for Qwen2b assuming grad_chunk=32. Do lower for others.       
        numtokens=model_inputs[0]['qry']['attention_mask'].shape[1]
        dynamic_chunk_size_qry = self.chunk_sizes[0]//self.multipliers_qry[bisect.bisect_left(self.thresholds_qry, numtokens)]
        numtokens=model_inputs[1]['tgt']['attention_mask'].shape[1]
        dynamic_chunk_size_pos = self.chunk_sizes[1]//self.multipliers_pos[bisect.bisect_left(self.thresholds_pos, numtokens)]

        
        self.dynamic_chunk_size=[dynamic_chunk_size_qry, dynamic_chunk_size_pos]
        
        logger.info(f"Dynamic GradCache chunk size: {self.dynamic_chunk_size}")

        model_inputs = [self.split_inputs(x, chunk_size) for x, chunk_size in zip(model_inputs, self.dynamic_chunk_size)]
        ...
        cache = [c.split(chunk_size) for c, chunk_size in zip(cache, self.dynamic_chunk_size)]
