...
def data_prepare(batch_dict, *args, **kwargs):
    ...
    for video_idx, query, answer_idx, question_idx, options in \
        ...
        video_path = f'{video_dir}/{video_idx}.mp4'
        frame_dir = f'{frame_basedir}/{video_idx}'
...
def load_egoschema_dataset(model_args, data_args, training_args, *args, **kwargs):
    ...
    if num_sample_per_subset is not None and type(num_sample_per_subset) is str:
    ...
    dataset = dataset.map(lambda x: data_prepare(x, **kwargs), batched=True, batch_size=64,
                          remove_columns=['question_idx', 'question', 'video_idx', 'option'],
