...
from src.data.utils.video_transforms_videochat2 import GroupScale, GroupCenterCrop
...
def data_prepare(batch_dict, *args, **kwargs):
    ...
    video_basedir = kwargs['video_dir']
    frame_basedir = kwargs['frame_dir']
    ...
    for row_idx, (subset, query, video_filename, cands, answer) in enumerate(
        ...
        video_path = f'{video_basedir}/{subset_meta_info["video_path"]}/{video_filename}'
        frame_dir = f'{frame_basedir}/{subset}/{video_filename}'
...
def load_mvbench_dataset(model_args, data_args, training_args, *args, **kwargs):
    subsets = []
    ...
    dataset = dataset.to_iterable_dataset(num_shards=8)  # convert to IterableDataset and multiple shards
    ...
    dataset = dataset.map(lambda x: data_prepare(x, **kwargs), batched=True, batch_size=64,
                          remove_columns=['video', 'question', 'candidates', 'answer', 'start', 'end', 'accurate_start', 'accurate_end', 'subtitle', 'show_name', 'fps'],
