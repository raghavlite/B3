...
def data_prepare(batch_dict, *args, **kwargs):
    ...
    for query, video_id, options, answer, question_id, domain, sub_category in (
        ...
        frame_dir = f'{frame_basedir}/{video_id}'
...
def load_videomme_dataset(model_args, data_args, training_args, *args, **kwargs):
    ...
    dataset = dataset.map(lambda x: data_prepare(x, **kwargs), batched=True, batch_size=64,
                          remove_columns=['video_id', 'duration', 'domain', 'sub_category', 'url', 'videoID', 'question_id', 'task_type', 'question', 'options', 'answer'],
