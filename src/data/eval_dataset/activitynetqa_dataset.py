...
def data_prepare(batch_dict, *args, **kwargs):
    ...
    for video_name, query, answer, question_id in \
        ...
        video_path = f'{video_dir}/v_{video_name}.mp4'
        frame_dir = f'{frame_basedir}/v_{video_name}'
...
def sub_sample(video_dir, video_export_dir):
    ...
    yesno_dataset.save_to_disk("/fsx/home/ruimeng/data/MMEB_Pro/data/ActivityNetQA/")
    with open(f'/fsx/home/ruimeng/data/MMEB_Pro/data/activitynetqa.jsonl', 'w') as f:
...
def load_activitynetqa_dataset(model_args, data_args, training_args, *args, **kwargs):
    ...
    dataset = dataset.to_iterable_dataset(num_shards=1)  # convert to IterableDataset and multiple shards
