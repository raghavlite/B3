...
class CR_EvalDataset(Dataset):
    def __init__(self, data_args, model_args, subset, text_field, img_path_field, mod_qinstruction=None, mod_cinstruction=None, add_qnl=False, add_cnl=False):
        """
        (text_field, image_field) -> ("qry_text", "qry_img_path") or ("tgt_text", "tgt_img_path")
        """
        self.data_args = data_args
        ...
        # )
        # self.paired_data = self.get_paired_data(text_field, img_path_field)
        if(text_field=='caption' and img_path_field==None):
            aa=subset[0]
            if isinstance(aa["caption"], str):
                kk=[mod_cinstruction+pair["caption"].strip() for pair in subset]    
            elif isinstance(aa["caption"], list):
                # handle list case   
                kk=[mod_cinstruction+caption.strip() for pair in subset for caption in pair["caption"]]
            else:
                assert False, "Bad datatype"
            if(add_cnl):
               kk=[pp+"\n" for pp in kk] 
            self.paired_dataset = datasets.Dataset.from_dict({
                "text": kk,
        ...
        elif(text_field==None and img_path_field=="img"):
            kk=[pair["img"] for pair in subset]
            if(add_qnl):
               mod_qinstruction=mod_qinstruction+"\n"
            self.paired_dataset = datasets.Dataset.from_dict({
                "text": [mod_qinstruction]*len(kk),
                "img_path": kk
            })
        else:
            assert False
        
        # if(mod_qinstruction is not None) :
        #     print("Using QRY mod", mod_qinstruction, flush=True)            
        #     self.paired_dataset = datasets.Dataset.from_dict({
        #                         "text": [mod_qinstruction for pair in self.paired_data],
        #                         "img_path": [pair["img_path"] for pair in self.paired_data]
        #                         })
        #     print(">>>>>>>>>>>>>inside tgt_mod_txt", flush=True)

        # elif(mod_cinstruction is not None) :
        #     print("Using TGT mod", mod_cinstruction, flush=True)            
        #     self.paired_dataset = datasets.Dataset.from_dict({
        #                         "text": [mod_cinstruction + pair["text"] for pair in self.paired_data],
        #                         "img_path": [pair["img_path"] for pair in self.paired_data]
        #                         })
        #     print(">>>>>>>>>>>>>inside tgt_mod_txt", flush=True)
        #     # import ipdb; ipdb.set_trace()
        # else:
        #     print("Not using TGT mod", flush=True)            
        #     self.paired_dataset = datasets.Dataset.from_dict({
        #                         "text": [pair["text"] for pair in self.paired_data],
        #                         "img_path": [pair["img_path"] for pair in self.paired_data]
        #                         })
