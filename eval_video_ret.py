...
def main():
    ...
    # Compute embeddings
    for idx, (dataset_name, task_config) in enumerate(dataset_configs.items()):
        ...
        if os.path.exists(score_path) or (os.path.exists(query_embed_path) and os.path.exists(cand_embed_path)):
            print(f"Found previous eval score or embeddings, skipping encoding for {dataset_name}")
            continue

        eval_qry_dataset, corpus = AutoPairDataset.instantiate(
        ...
        all_video_id = []
        with torch.no_grad():
            for cand_inputs, cand_dataset_info in tqdm(eval_cand_loader, desc=f"Encoding candidates - {dataset_name}"):
                ...
                for info in cand_dataset_info:
                    all_video_id.append(info["cand_name"])
        cand_embeddings = np.concatenate(cand_embeddings)
        for embed, video_id in zip(cand_embeddings, all_video_id):
